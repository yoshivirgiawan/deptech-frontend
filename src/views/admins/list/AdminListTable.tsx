...
// Type Imports
import type { AdminType } from '@/types/pages/admins/adminTypes'
...
import { deleteAdminWithToast, fetchAllAdmins } from '@/redux-store/slices/admins'
...
type AdminTypeWithAction = AdminType & {
  action?: string
}
...
// Column Definitions
const columnHelper = createColumnHelper<AdminTypeWithAction>()
...
const AdminListTable = () => {
  ...
  const columns = useMemo<ColumnDef<AdminTypeWithAction, any>[]>(
    () => [
          ...
          <div className='flex items-center gap-3'>
            <div className='flex flex-col items-start'>
              <Typography
                component={Link}
                color='text.primary'
                href={getLocalizedUrl(`/admins/details/${row.original.id}`, locale as Locale)}
      ...
      columnHelper.accessor('action', {
        ...
        cell: ({ row }) => (
          <div className='flex items-center'>
            <IconButton LinkComponent={Link} href={`/${locale}/admins/detail/${row.original.id}`}>
              <i className='tabler-folder-open text-textSecondary' />
            </IconButton>
            <IconButton LinkComponent={Link} href={`/${locale}/admins/edit/${row.original.id}?editfromlist=true`}>
  ...
  return (
    <>
      {store.loading ? (
        <>
          <PageHeader title='Admins Page' hideActionButton />
